...
refresh_actions(Actions, Pred) ->
    lists:foreach(
        fun(ActionInst) ->
            logger:error("ActionInst ~p,Pred ~p", [ActionInst, Pred]),
            case ActionInst of
                #action_instance{args = Args, id = Id, name = ActName, fallbacks = Fallbacks} ->
